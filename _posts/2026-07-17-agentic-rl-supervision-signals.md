...
Agent 训练是一套随策略更新而变化的闭环实验。Policy 决定进入哪些状态，Verifier 把环境结果压缩成 Reward，Credit Assignment 再把轨迹级结果分配给具体决策；其中的偏差会经后续 Rollout 逐步放大。可靠训练需要持续采样当前策略的经验，在变化的数据分布上校准 Verifier，并通过过程信号和同状态分支提高归因精度。Mid-training 与 SFT 提供接口能力和行为先验，系统约束守住安全边界，RL 在可验证环境中优化终局结果。训练能否持续扩展，取决于单位环境成本能够产生多少可靠、可归因的经验。
