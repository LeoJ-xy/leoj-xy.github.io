...
excerpt: "从 Reward、State、Credit 三个坐标理解 Agent 训练闭环：Reward 决定学习方向，State 决定训练发生的位置，Credit 决定结果归属于哪些行动。"
tags:
  - Post-training
  - Agents
...
设想一个 Coding Agent 接到任务：修复某个偶发的并发 Bug。

它搜索了 12 次代码，读了 8 个文件，提出 3 个假设，修改两处实现，运行 6 轮测试，最后让隐藏测试全部通过。
...
- 哪次搜索提供了关键证据？
- 第一个假设虽然错误，有没有帮助排除一条路径？
- 两处修改中，哪一处修复了根因？
- 反复运行的 4 次测试是否纯属浪费？
- 如果测试通过来自 Hardcode 或篡改测试，成功标签还可信吗？
- Agent 失败时，哪些中间步骤依然值得保留？

一次文本生成通常只有输入与目标输出。Agent 的动作会改变环境，环境变化又会改写后续输入。训练对象由一个答案扩展成了一条闭环轨迹：
...
这个形式揭示了一个关键点：Agent 的策略既是学习者，也是数据生成机制的一部分。策略一变，它访问的状态、获得的观察、犯下的错误都会变化。
...
训练更新 $$\pi$$ 后，下一轮采样的 $$d_E^\pi$$ 也跟着改变。模型在学习如何行动，同时不断制造新的训练分布。

这意味着 Agent 训练是一套反馈架构。它有两个职责：

- **测量**：判断一条轨迹给环境带来了什么价值；
- **干预**：在当前策略实际会遇到的状态上提供学习信号。

沿着这个闭环，训练设计可以直接归结为 Reward、State、Credit 三个问题：

1. **Reward：策略究竟应该最大化什么？**  
   Reward 把真实任务压缩成优化目标，决定学习方向，也决定 Reward Hacking 会从哪里出现。

2. **State：训练数据是否覆盖策略实际到达的状态？**  
   State 决定训练覆盖哪一段 Occupancy Distribution，也决定教师示范能否迁移到部署。

3. **Credit：延迟结果应该归因给哪些决策？**  
   Credit 决定一条长轨迹中的哪些 Action 得到强化，哪些 Action 需要修正。

全文依次讨论这三个问题。SFT、OEC、RL、Process Reward 与 Branched Rollout，分别提供不同的信息来源和训练方式。
...
对于普通问答，真值可能是一个标准答案。对于 Agent，结果通常表现为世界状态发生变化：
...
因此，高质量 Reward 通常直接读取环境状态。Agent 对过程的描述更适合作为辅助信号。
...
Coding 领域进展快，原因就在这里。编译器、单元测试、静态分析、文件 Diff 和隐藏测试共同提供了较强的测量体系。Web 和企业流程也可以检查 URL、数据库记录或业务对象状态。

### 2.2 RLVR 没有消除目标定义问题

程序化 Reward 比 Learned Reward Model 稳定，但 Checker 仍可能测错东西。

[SWE-RL](https://arxiv.org/abs/2502.18449) 的训练 Reward 使用预测 Patch 与标准 Patch 的文本相似度。它便宜且稠密，却会惩罚语义等价的另一种修复。这里测量的是“像不像标准答案”，程序正确性没有被直接验证。

测试也可能覆盖不全。Agent 可以修改测试、Hardcode 可见样例，或利用 Parser 漏洞。OpenAI 对 SWE-Bench Pro 的审计发现约三成任务存在问题；[BenchJack](https://arxiv.org/html/2605.12673) 在 10 个 Agent Benchmark 中找到 219 个可利用漏洞。

所以 RLVR 把 Specification Problem 推到了 Verifier 层。奖励具有确定性，仍可能测错真实目标。
